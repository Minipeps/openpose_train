...
import os
import sys
from getResNetProtoTxt import getResNet50Init
from getDenseNetProtoTxt import getDenseNet121Init, getDenseNet169Init, getDenseNet201Init, getDenseNet161v2Init
...
def generateProtoTxt(#dataFolders,
                     trainingFolder, batchSizes, layerName, kernel, stride, numberOutputChannels,
                     ...
                     numberIterations, maximumPafStage, usePReLU, extraGT, noBodyParts, noBodyPAFs, distanceChannels,
                     addBkgChannel):
    # pose_training.prototxt - Training prototxt
    stringToWrite = setLayersTwoBranches(#dataFolders,
                                         batchSizes, layerName, kernel, stride, numberOutputChannels,
                                         numberKeyPoints, numberPAFs, transformParams, False, batchNorm,
                                         binaryConv, lrMultDistro, caffeFolder, isFinalModel, maximumPafStage,
                                         usePReLU, extraGT, noBodyParts, noBodyPAFs, distanceChannels,
                                         addBkgChannel)
    with open('%s/pose_training.prototxt' % trainingFolder, 'w') as f:
        f.write(stringToWrite)

    # pose_deploy.prototxt - Deployment prototxt
    stringToWrite = setLayersTwoBranches(#[],
                                         [], layerName, kernel, stride, numberOutputChannels, 
                                         numberKeyPoints, numberPAFs, transformParams, True, batchNorm,
                                         binaryConv, lrMultDistro, caffeFolder, isFinalModel, maximumPafStage,
                                         usePReLU, extraGT, noBodyParts, noBodyPAFs, distanceChannels,
                                         addBkgChannel)
    ...
    solverString = getSolverPrototxt(learningRateInit, numberIterations, trainedModelsFolder, transformParams)
...
def setLayersTwoBranches(#dataFolders,
                         batchSizes, layerName, kernel, stride, numberOutputChannels,
                         numberKeyPoints, numberPAFs, transformParams, deploy, batchNorm, binaryConv,
                         lrMultDistro, caffeFolder, isFinalModel, maximumPafStage, usePReLU, extraGT,
                         noBodyParts, noBodyPAFs, distanceChannels, addBkgChannel):
    # Caffe includes
    ...
    # Training mode - Use lmdb
        ...
        # If merging different datasets
            ...
            for index in range(0, len(batchSizes)):
                ...
                caffeNet.tops['data' + str(index)], caffeNet.tops['label' + str(index)] = L.OPData(
                    data_param=dict(backend=1, #source=dataFolders[index],
                    batch_size=batchSizes[index]),
        ...
        # Slice layer
        if noBodyParts + noBodyPAFs > 0:
            temp = '_temp' # For foot, also enable below code
            # temp = ''
        else:
            temp = ''
        # Order from OPDataLayer: L2, L1, L3
        # Desired order:          L1, L2, L3
        # With distance
        if distanceChannels > 0:
            caffeNet.tops[sLabelName[sLNSize+1]+temp], caffeNet.tops[sLabelName[sLNSize]+temp], caffeNet.tops[sLabelName[sLNSize+2]+temp], caffeNet.tops[sLabelName[2*sLNSize+1]+temp], caffeNet.tops[sLabelName[2*sLNSize]+temp], caffeNet.tops[sLabelName[2*sLNSize+2]+temp] = L.Slice(
                caffeNet.label,
                slice_param=dict(axis=1, slice_point=[numberPAFs+noBodyPAFs, numberTotalParts+1*addBkgChannel, distanceChannels+numberTotalParts+1*addBkgChannel, distanceChannels+numberTotalParts+numberPAFs+noBodyPAFs+1*addBkgChannel, distanceChannels+2*numberTotalParts+2*1*addBkgChannel]), # For foot, also enable below code
                # slice_param=dict(axis=1, slice_point=[noBodyPAFs, noBodyPAFs+noBodyParts, 2*noBodyPAFs+noBodyParts]), # For hands
                ntop=6
            )
        # Without distance
        else:
            ...
            for index in range(2,6):
                ...
                if index%2==0:
                    caffeNet.tops[sLabelName[index]+'_1'], caffeNet.tops[sLabelName[index]], caffeNet.tops[sLabelName[index]+'_2'] = L.Slice(
                        ...
                        slice_param=dict(axis=1, slice_point=[bodyNumber,bodyNumber+noBodyNumber]),
                        ntop=3
                    )
                    caffeNet.tops[sLabelName[index]+'_silent2'] = L.Silence(caffeNet.tops[sLabelName[index]+'_2'], ntop=0)
                else:
                    caffeNet.tops[sLabelName[index]+'_1'], caffeNet.tops[sLabelName[index]] = L.Slice(
                        caffeNet.tops[sLabelName[index]+temp],
                        slice_param=dict(axis=1, slice_point=[bodyNumber]),
                        ntop=2
                    )
                caffeNet.tops[sLabelName[index]+'_silent1'] = L.Silence(caffeNet.tops[sLabelName[index]+'_1'], ntop=0)

        # extraGT
        if extraGT and (noBodyParts + noBodyPAFs > 0):
            raise ValueError("Not implemented for this case.")
        if extraGT:
            caffeNet.tops[sLabelName[sLNSize+1]+'_2'], caffeNet.tops[sLabelName[sLNSize]+'_2'], caffeNet.tops[sLabelName[2*sLNSize+1]+'_2'], caffeNet.tops[sLabelName[2*sLNSize]+'_2'] = L.Slice(
                caffeNet.tops['label_2'],
                slice_param=dict(axis=1, slice_point=[numberPAFs+noBodyPAFs, numberTotalParts+1*addBkgChannel, numberTotalParts+numberPAFs+noBodyPAFs+1*addBkgChannel]),
                ntop=4
            )
        # 2 Eltwise layers
        # With distance
        if distanceChannels > 0:
            for level in range(0,3):
                caffeNet.tops[sLabelName[level]] = L.Eltwise(caffeNet.tops[sLabelName[level+sLNSize]],
                                                             caffeNet.tops[sLabelName[level+2*sLNSize]],
                                                             operation=P.Eltwise.PROD)
                if extraGT:
                    caffeNet.tops[sLabelName[level]+'_2'] = L.Eltwise(caffeNet.tops[sLabelName[level+sLNSize]+'_2'],
                                                                      caffeNet.tops[sLabelName[level+2*sLNSize]+'_2'],
                                                                      operation=P.Eltwise.PROD)
        # Without distance
        else:
    ...
    for l in range(0, len(layerName)):
        ...
        if layerName[l] == 'V': #pretrained VGG layers
            ...
            if (usePReLU == 1 and layerName[l+1][0] == 'C'):
                ReLUname = 'p' + ReLUname
                caffeNet.tops[ReLUname] = L.PReLU(
                    caffeNet.tops[lastLayer], param=[dict(lr_mult=lr_m, decay_mult=1)], channel_shared=False, in_place=True)
            # LReLU
            elif (usePReLU == 2 and layerName[l+1][0] == 'C'):
                ReLUname = 'l' + ReLUname
                caffeNet.tops[ReLUname] = L.ReLU(caffeNet.tops[lastLayer], negative_slope=0.25, in_place=True)
            # ReLU
            else:
        ...
        # Convolution layers
            ...
            if kernel[l] == 1:
                ...
                caffeNet.tops[concatName] = L.Concat(caffeNet.tops["relu4_2"], caffeNet.tops["relu4_4"],
                                                     caffeNet.tops[halfLayer], caffeNet.tops[lastLayer], concat_param=dict(axis=1))
            ...
            if layerName[l+1] != 'L':
                if state == 'image':
                    ...
                    # LReLU
                        ...
                        ReLUname = 'Mlrelu%d_stage%d' % (convCounter, stage)
                        caffeNet.tops[ReLUname] = L.ReLU(caffeNet.tops[lastLayer], negative_slope=0.25, in_place=True)
                    # ReLU
                    else:
            ...
            localCounter += 1
        # Convolution layers
        elif len(layerName[l]) == 2 and layerName[l] == 'DC':
            if state == 'image':
                convName = 'conv%d_%d_CPM' % (poolCounter, localCounter) # no image state in subsequent stages
                lr_m = lrMultDistro[1] * (isFinalModel or stage == maximumPafStage)
            else: # fuse
            ...
            for index in range(0,3):
                ...
                if layerName[l+1] != 'L':
                    ...
                    if usePReLU == 1:
                        if state == 'image':
                            ReLUname = 'prelu%d_%d_CPM_%d' % (poolCounter, localCounter, index)
                        else:
                            ReLUname = 'Mprelu%d_stage%d_%d' % (convCounter, stage, index)
                        if not isFinalModel:
                            ReLUname += '_temp'
                    ...
                    # LReLU
                        ...
                        if state == 'image':
                            ReLUname = 'lrelu%d_%d_CPM_%d' % (poolCounter, localCounter, index)
                        else:
                            ReLUname = 'Mlrelu%d_stage%d_%d' % (convCounter, stage, index)
                        ReLUname = init + ReLUname
                        caffeNet.tops[ReLUname] = L.ReLU(caffeNet.tops[lastLayer], negative_slope=0.25, in_place=True)
                    # ReLU
                    else:
                        if state == 'image':
                            ReLUname = 'relu%d_%d_CPM_%d' % (poolCounter, localCounter, index)
                        else:
                            ReLUname = 'Mrelu%d_stage%d_%d' % (convCounter, stage, index)
                        if not isFinalModel:
                            ReLUname += '_temp'
                        ReLUname = init + ReLUname
                        caffeNet.tops[ReLUname] = L.ReLU(caffeNet.tops[lastLayer], in_place=True)
        ...
        #     - Hand: 'DC1h', 'DC2h', 'DC3h'
            ...
            for index in range(0,3):
                ...
                # LReLU
                    ...
                    if state == 'image':
                        ReLUname = 'lrelu%d_%d_CPM_L%d_%d' % (poolCounter, localCounter, level+1, index)
                    else:
                        ReLUname = 'Mlrelu%d_stage%d_L%d_%d' % (convCounter, stage, level+1, index)
                    ReLUname = init + ReLUname
                    caffeNet.tops[ReLUname] = L.ReLU(caffeNet.tops[lastLayer], negative_slope=0.25, in_place=True)
                # ReLU
                else:
            ...
            # # Batch Norm + scale
            # if convCounter % 2 == 0: # Assuming it starts in 1
            # # if convCounter % 2 == 1: # Assuming it starts in 0
            #     batchNormName = 'Mbn%d_stage%d_L%d' % (convCounter, stage, level+1)
            #     caffeNet.tops[batchNormName] = L.BatchNorm(caffeNet.tops[lastLayer], in_place=True)
            #     scaleNormName = 'Mscale%d_stage%d_L%d' % (convCounter, stage, level+1)
            #     caffeNet.tops[scaleNormName] = L.Scale(caffeNet.tops[batchNormName], bias_term=True, in_place=True)
        ...
        #             caffeNet.tops[lastLayer], caffeNet.tops[sLabelName[(level+sLNSize)]], operation=P.Eltwise.PROD)
        #         caffeNet.tops['loss_stage%d' % stage] = L.EuclideanLoss(
        #             caffeNet.tops[name], caffeNet.tops[sLabelName[level]])
        ...
        #     state = 'image'
            ...
            if not deploy:
                ...
                caffeNet.tops[finalLossName] = L.Concat(caffeNet.tops[sLabelName[0]], caffeNet.tops[sLabelName[1]], concat_param=dict(axis=1))
                # Weights
                finalWeightName = 'final_weights'
                caffeNet.tops[finalWeightName] = L.Concat(caffeNet.tops[sLabelName[sLNSize]], caffeNet.tops[sLabelName[sLNSize+1]], concat_param=dict(axis=1))
        ...
        #     - 'L1c', 'L2c', 'L3c'
            ...
            if not deploy and not isFakeDeploy:
                ...
                if not extraGT or len(layerName[l]) < 4:
                    # Eltwise product
                    caffeNet.tops[name] = L.Eltwise(
                        caffeNet.tops[previousLayer], caffeNet.tops[sLabelName[(level+sLNSize)]], operation=P.Eltwise.PROD)
                    # Euclidean loss
                    caffeNet.tops['loss_stage%d_L%d' % (stage, level+1)] = L.EuclideanLoss(
                        caffeNet.tops[name], caffeNet.tops[sLabelName[level]], loss_weight=weight)
                else:
                    caffeNet.tops[name] = L.Eltwise(
                        caffeNet.tops[previousLayer], caffeNet.tops[sLabelName[(level+sLNSize)]+'_2'], operation=P.Eltwise.PROD)
                    caffeNet.tops['loss_stage%d_L%d' % (stage, level+1)] = L.EuclideanLoss(
        ...
        #     - Upsampling: '@U'
            ...
            if layerName[l] == '@' or isFoot or isHand:
                if lastLayerL1 != '' and lastLayerL2 != '' and lastLayerL3 != '' and lastLayerL1f and lastLayerL2f:
                    caffeNet.tops[nextLayer] = L.Concat(
                        caffeNet.tops[sharePoint], caffeNet.tops[lastLayerL1], caffeNet.tops[lastLayerL2], caffeNet.tops[lastLayerL3], caffeNet.tops[lastLayerL1f], caffeNet.tops[lastLayerL2f], concat_param=dict(axis=1))
                elif lastLayerL1 != '' and lastLayerL2 != '' and lastLayerL1f and lastLayerL2f:
                    caffeNet.tops[nextLayer] = L.Concat(
                ...
                elif lastLayerL1 != '' and lastLayerL2 != '' and lastLayerL3 != '':
                    caffeNet.tops[nextLayer] = L.Concat(
                        caffeNet.tops[sharePoint], caffeNet.tops[lastLayerL1], caffeNet.tops[lastLayerL2], caffeNet.tops[lastLayerL3], concat_param=dict(axis=1))
    ...
    if deploy:
        ...
        if lastLayerL1f != '' and lastLayerL2f != '':
            # caffeNet.tops[lastLayerL1+'_body'], caffeNet.tops[lastLayerL1+'_bkg'] = L.Slice(
            #     caffeNet.tops[lastLayerL1],
            #     slice_param=dict(axis=1, slice_point=[numberKeyPoints]),
            #     ntop=2
            # )
            # caffeNet.tops['net_output'] = L.Concat(
            #     caffeNet.tops[lastLayerL1+'_body'], caffeNet.tops[lastLayerL1f], caffeNet.tops[lastLayerL1+'_bkg'],
            #     caffeNet.tops[lastLayerL2], caffeNet.tops[lastLayerL2f], concat_param=dict(axis=1))
            caffeNet.tops['net_output'] = L.Concat(
        ...
        elif lastLayerL1h != '' and lastLayerL2h != '':
            caffeNet.tops[lastLayerL1+'_body'], caffeNet.tops[lastLayerL1+'_bkg'] = L.Slice(
                caffeNet.tops[lastLayerL1],
                slice_param=dict(axis=1, slice_point=[numberKeyPoints]),
                ntop=2
            ...
            caffeNet.tops['net_output'] = L.Concat(
                caffeNet.tops[lastLayerL1+'_body'], caffeNet.tops[lastLayerL1h], caffeNet.tops[lastLayerL1+'_bkg'],
                caffeNet.tops[lastLayerL2], caffeNet.tops[lastLayerL2h], concat_param=dict(axis=1))
            # caffeNet.tops['net_output'] = L.Concat(
            #     caffeNet.tops[lastLayerL1], caffeNet.tops[lastLayerL1h], caffeNet.tops[lastLayerL2], caffeNet.tops[lastLayerL2h], concat_param=dict(axis=1))
        elif lastLayerL3 != '':
            caffeNet.tops['net_output'] = L.Concat(
                caffeNet.tops[lastLayerL1], caffeNet.tops[lastLayerL2], caffeNet.tops[lastLayerL3], concat_param=dict(axis=1))
        else:
            caffeNet.tops['net_output'] = L.Concat(
                caffeNet.tops[lastLayerL1], caffeNet.tops[lastLayerL2], concat_param=dict(axis=1))
        # # Deconvolution layer
        # if True:
        #     factor = 2
        #     numberChannels = numberTotalParts+1*addBkgChannel
...
input_dim: {} # This value will be defined at runtime\n'.format('"' + input + '"', dim1, dim2, dim3, dim4)
        ...
        # # DenseNet style
        # return deployInit + getDenseNet121Init() + layerText + layerText.join(str(caffeNet.to_proto()).split(layerText)[2:])
        # # return deployInit + getDenseNet161v2Init() + layerText + layerText.join(str(caffeNet.to_proto()).split(layerText)[2:])
    else:
        # VGG style
        ...
        # # DenseNet style
        # return getDenseNet121Init() + str(caffeNet.to_proto())
        # # return getDenseNet161v2Init() + str(caffeNet.to_proto())



def getSolverPrototxt(learningRateInit, numberIterations, snapshotFolder, transformParams):
    # try:
    #     probSecond = transformParams[0]['prob_secondary']
    #     stepRatio = min(1.0/probSecond, 1.0/(1.0-probSecond))
    #     print 'Solver stepRatio = ' + str(stepRatio)
    # except:
...
# stepvalue: 90000\n\
# stepvalue: 100000\n\
stepvalue: ' + str(int(round(200000*stepRatio))) + '\n\
stepvalue: ' + str(int(round(300000*stepRatio))) + '\n\
stepvalue: ' + str(int(round(360000*stepRatio))) + '\n\
stepvalue: ' + str(int(round(420000*stepRatio))) + '\n\
stepvalue: ' + str(int(round(480000*stepRatio))) + '\n\
stepvalue: ' + str(int(round(540000*stepRatio))) + '\n\
stepvalue: ' + str(int(round(600000*stepRatio))) + '\n\
stepvalue: ' + str(int(round(700000*stepRatio))) + '\n\
stepvalue: ' + str(int(round(800000*stepRatio))) + '\n\
stepvalue: ' + str(int(round(900000*stepRatio))) + '\n\
